    ...
    <div className="min-h-screen bg-[var(--background)] text-ink">
      <div className="max-w-[900px] mx-auto px-6 py-10">
        <div className="flex items-start justify-between mb-8 gap-6 flex-wrap">
          <div>
            <div className="font-heading text-2xl font-extrabold">How Chess-2000 Works</div>
          ...
          <div className="flex gap-2 flex-wrap">
            <Link href="/Chess-2000" className="text-[12px] font-bold bg-panel px-3 py-2 whitespace-nowrap">About & Pricing</Link>
            <Link href="/Chess-2000/play" className="text-[12px] font-bold bg-panel px-3 py-2 whitespace-nowrap">← Back to game</Link>
          </div>
        </div>
        ...
        <Section title="How to use this website">
          ...
          <p>The board itself supports both drag-and-drop and click-to-move: click a piece to see its real legal destinations marked with dots (a plain dot for a quiet move, a ring for a real capture), then click a destination, or just drag the piece there directly. The eight-button toolbar above the board — Undefended Pieces, Overloaded Pieces, Weak Squares, Piece Strength, Imbalance, Exchange Alert, Tactics Alert, and Checkmate Alert — each highlight one real, specific thing on the board and explain it in the Positional Analysis box on the right. Click a tool to see it; click it again to go back to the general position summary. Two buttons flank the board itself: Hint on the left (the engine's real best move for your own turn only) and Back on the right (undoes your last move and the engine's reply to it).</p>
          ...
          <p>The Positional Analysis box in the bottom-right corner is worth understanding on its own: with no toolbar button selected, it writes out the position the way a coach actually would, in priority order rather than a fixed checklist — whatever is genuinely most important right now (a forced mate, a hanging piece, a real threat, a real weak point) leads the explanation, and anything not actually relevant this move (an unthreatened king, for instance) is left out rather than mentioned out of habit. It also names what the opponent's real plan is right now, and closes with a real reminder to review the move you just played before moving on. The moment you select any toolbar tool, that same box switches to explaining exactly what that tool found, then switches back the moment you deselect it. It is designed to always be showing you something real and specific, never a placeholder.</p>
        </Section>

        <Section title="How to train toward 2000 ELO with this method">
          <p>2000 ELO is not a tactical-wizard rating. At club level, the single biggest gap between a 1000-rated player and a 2000-rated player is not calculation depth or opening theory — it is consistency. A 2000 player is, in a very real sense, a 1000 player who has trained themselves out of hanging pieces and out of missing the opponent's real threats. Chess-2000 is built around exactly that gap, using the CCTAP method (explained in full below) as the real, repeatable habit that closes it.</p>
          <p>The training loop is simple and is meant to be repeated every single move, not just when something looks dangerous: before you move, run through Checks, Captures, Threats, and Analysis using the real toolbar data, then commit to a Plan. Early on, use CCTAP Mode (the header button) to walk through the five steps explicitly, one at a time — each step tells you which real toolbar tool actually answers that step's question, so you check it yourself rather than the app switching it for you. As the habit becomes automatic, you'll find yourself doing it without the guided mode — which is the actual goal; CCTAP Mode is training wheels, not a permanent crutch.</p>
        ...
        <Section title="The CCTAP method, explained with examples">
          ...
          <p><strong className="text-ink">Checks.</strong> Can you give check right now, and does it lead anywhere? Example: your rook is on the e-file, the enemy king is on e8 with nothing blocking the file — Re1+ is a real check worth calculating fully, even if it turns out not to be immediately winning, because forcing moves narrow the opponent's real options and can reveal follow-up tactics. In Chess-2000, the Checkmate Alert tool answers this directly: it shows a real forced mate if one exists, in either direction, so you always know if this step actually matters right now.</p>
          ...
          <p>CCTAP Mode in the app walks through exactly these five steps, each with its own real, concrete "what to do this step, and which real tool actually answers it" guidance — a real, guided version of the same process described above, meant to be trained until it becomes automatic without the guide. CCTAP Mode deliberately doesn't switch the toolbar for you (a real, reported bug otherwise: it used to, and it hijacked whatever the Positional Analysis panel was showing without being asked) — the guidance names the tool, and you check it yourself when you want the live data for that step.</p>
        ...
        <Section title="Glossary">
          <p>Every real term used across Chess-2000, what it actually means, and exactly how it's calculated — no black boxes.</p>
        ...
        <div className="mt-10 pt-6 border-t border-panel-line">
          <Link href="/Chess-2000/play" className="text-[12px] font-bold bg-ink text-[var(--background)] px-4 py-2 inline-block">← Back to game</Link>
        </div>
      </div>
    </div>
  )
